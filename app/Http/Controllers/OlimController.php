...
class OlimController extends Controller
{
    ...
    public function index()
    {
        ...
        'nama_bayar', 'price', 'olim.updated_at', 'asal_info')
    ...
    public function hapus(Request $request)
    {
        ...
        $data = Olim::where('id', $timId)->get()->first();
        User::where('id', $data->userid)->update(['daftar' => null]);
        Olim::where('id', $timId)->delete();
    ...
    {
        $timId = $request->timId;
        $status = Olim::where('id', $timId)->get(['status'])->first(); //get user verif status
        if ($status->status == false) {
            Olim::where('id', $timId)->update(['status' => true, 'no_peserta' => $request->noPeserta]);
        } else {
            Olim::where('id', $timId)->update(['status' => false]);
        }
        return redirect("/daftarolim");
    }
    ...
    {
        $userId = Auth::user()->id;
        $kode = Kode::where('name', 'olim')->get()->first();
        Olim::where('userid', $userId)->update(['no_peserta' => 'OLI0'.$kode->no, 'is_submitted'=>true]);

        Kode::where('name', 'olim')->update(['no' => $kode->no+1]);
