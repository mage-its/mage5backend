...
class EsportController extends Controller
{
    ...
    public function index()
    {
        ...
        'nama_bayar', 'price', 'esport.updated_at', 'asal_info')
    ...
    public function hapus(Request $request)
    {
        ...
        $data = Esports::where('id', $timId)->get()->first();
        User::where('id', $data->userid)->update(['daftar' => null]);
        Esports::where('id', $timId)->delete();
    ...
    {
        $timId = $request->timId;
        $status = Esports::where('id', $timId)->get(['status'])->first(); //get user verif status
        if ($status->status == false) {
            Esports::where('id', $timId)->update(['status' => true, 'no_peserta' => $request->noPeserta]);
        } else {
            Esports::where('id', $timId)->update(['status' => false]);
        }
        return redirect("/daftaresport");
    }
    ...
    public function submit(Request $request)
    {
        ...
        switch($request->cabang_lomba) {
            case 4: $cabang = 'ml'; $prefix = 'EPM0'; break;
        }
